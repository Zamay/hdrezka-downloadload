...
class HdRezkaApi():
    ...
    def clearTrash(data):
        ...
        for i in range(2, 4):
            for chars in product(trashList, repeat=i):
                data_bytes = "".join(chars).encode("utf-8")
    ...
    def getTranslations(self):
        ...
        if not arr:
            ...
            def getTranslationID(s):
                initCDNEvents = {'video.tv_series': 'initCDNSeriesEvents', 'video.movie': 'initCDNMoviesEvents'}
    ...
    def getStream(self, season=None, episode=None, translation=None, index=0):
        season = self.frag[1] if self.frag is not None else None
        episode = self.frag[2] if self.frag is not None else None
        translation = self.frag[0] if self.frag is not None else None
        ...
        def makeRequest(data):
            ...
            if r['success']:
                ...
                stream = HdRezkaStream(season, episode, subtitles={'data': r['subtitle'], 'codes': r['subtitle_lns']})
                for i in arr:
